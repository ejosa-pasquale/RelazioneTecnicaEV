...
stile_relazione = st.radio(
    ...
    ["Lean (utile e snello)", "Completo (dettagliato)"],
    index=0,
    key="stile_relazione",
    help="Lean: mantiene struttura e rigore, evitando ripetizioni e parti raramente richieste. Completo: include anche i paragrafi tecnici estesi."
)
...
st.subheader("Fonti dati e prescrizioni (per evitare '' nel PDF)")
c1, c2 = st.columns(2)
with c1:
...
# =========================
# DATI DI PROGETTAZIONE (RILIEVO)
# =========================
st.subheader("Dati raccolti in fase di progettazione (rilievo e ipotesi)")
c1, c2, c3 = st.columns(3)
with c1:
    data_sopralluogo = st.date_input("Data sopralluogo (se effettuato)", value=None, key="data_sopralluogo")
    referente = st.text_input("Referente / contatto in sito (opzionale)", "", key="referente_sito")
with c2:
    fornitura = st.text_input("Fornitura / POD / Distributore (opzionale)", "", key="fornitura")
    protezione_monte = st.text_input("Protezione a monte (se nota) - tipo/In/PdI", "", key="prot_monte")
with c3:
    icc_presunta = st.text_input("Icc presunta al punto (se disponibile)", "", key="icc_presunta")
    ra_misurata = st.text_input("Ra misurata / stimata (se disponibile)", "", key="ra_misurata")
vincoli = st.text_area("Vincoli / note di rilievo (percorsi, attraversamenti, limiti, ecc.)", "", height=80, key="vincoli")
doc_disponibili = st.text_area("Documentazione disponibile (schemi esistenti, DiCo, libretti, ecc.)", "", height=80, key="doc_disp")
st.divider()

# =========================
# INFRASTRUTTURA EV (se pertinente)
# =========================
st.subheader("Infrastruttura di ricarica EV (se pertinente)")
evse_df = pd.DataFrame([
    {"Apparecchiatura":"Wallbox/Colonnina", "Marca/Modello":"", "Potenza_kW":7.4, "Alimentazione":"Monofase", "Modo ricarica":"3", "Connettore":"Tipo 2", "IP":"", "IK":"", "RCD richiesto":"Tipo A + RDC-DD / Tipo B (secondo manuale)", "Note":""},
])
evse_df = st.data_editor(evse_df, num_rows="dynamic", use_container_width=True, key="evse")
st.divider()

# =========================
# DOCUMENTAZIONE FOTOGRAFICA
# =========================
st.subheader("Documentazione fotografica (opzionale)")
foto_files = st.file_uploader("Carica foto (JPG/PNG) - max 6 consigliate", type=["jpg","jpeg","png"], accept_multiple_files=True, key="foto")
st.caption("Le foto caricate verranno inserite nel PDF solo se presenti (max 6).")
st.divider()

# =========================
# CHECKLIST DOCUMENTALE
# =========================
st.subheader("Checklist documentale (DM 37/08, DPR 462/01, consegna elaborati)")
checklist_df = pd.DataFrame([
    {"Documento/Elaborato":"Dichiarazione di Conformità (DiCo) DM 37/08", "Stato":"Da produrre", "Note":""},
    {"Documento/Elaborato":"Relazione tipologica e materiali impiegati (DM 37/08)", "Stato":"Da produrre", "Note":""},
    {"Documento/Elaborato":"Schema/planimetria impianto (unifilare/multifilare) (DM 37/08)", "Stato":"Da produrre", "Note":""},
    {"Documento/Elaborato":"Libretto uso/manutenzione apparecchiature (se previsto)", "Stato":"Consigliato", "Note":""},
    {"Documento/Elaborato":"Verbali prove e misure (CEI 64-8 Parte 6 / CEI 64-14)", "Stato":"Da produrre", "Note":""},
    {"Documento/Elaborato":"Denuncia impianto di terra / verifiche periodiche (DPR 462/01) (se applicabile)", "Stato":"Da verificare", "Note":""},
    {"Documento/Elaborato":"Schede tecniche / dichiarazioni CE componenti principali", "Stato":"Consigliato", "Note":""},
    {"Documento/Elaborato":"Report fotografico essenziale", "Stato":"Consigliato", "Note":""},
])
checklist_df = st.data_editor(
    ...
    use_container_width=True,
    key="checklist",
    column_config={"Stato": st.column_config.SelectboxColumn("Stato", options=["Presente","Da produrre","Da verificare","Non applicabile","Consigliato"], required=True)},
)
st.divider()
...
{("Integrazioni: " + criterio_note) if criterio_note.strip() else ""}"""
    ...
    # =========================
    # NORMALIZZAZIONE TABELLE AGGIUNTIVE
    # =========================
    def _row_meaningful(d: dict, keys):
        for k in keys:
            v = str(d.get(k, "")).strip()
            if v and v not in {"—","-"}:
                return True
    ...
    evse_list = []
    try:
        for _, r in evse_df.iterrows():
            d = r.to_dict()
            if _row_meaningful(d, ["Marca/Modello","Potenza_kW","Note"]):
                evse_list.append(d)
    except Exception:
        evse_list = []
    ...
    try:
        for _, r in checklist_df.iterrows():
            d = r.to_dict()
            if _row_meaningful(d, ["Documento/Elaborato","Stato","Note"]) and str(d.get("Stato","")).strip() != "":
                checklist_list.append(d)
    except Exception:
        checklist_list = []
    ...
    if foto_files:
        for f in foto_files[:6]:
            try:
                foto_bytes_list.append({"name": f.name, "bytes": f.getvalue()})
            except Exception:
                pass

    progettazione = {
        "data_sopralluogo": (data_sopralluogo.strftime("%d/%m/%Y") if data_sopralluogo else ""),
        "referente": referente,
        "fornitura": fornitura,
        "protezione_monte": protezione_monte,
        "icc_presunta": icc_presunta,
        "ra_misurata": ra_misurata,
        "vincoli": vincoli,
        "doc_disponibili": doc_disponibili,
    }
    ...
    payload = {
        ...
        "stile_relazione": stile_relazione,
        "progettazione": progettazione,
        "evse": evse_list,
        "checklist_documentale": checklist_list,
        "foto": foto_bytes_list,
    }
